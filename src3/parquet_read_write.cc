...
arrow::Status ReadFullFile(std::string path_to_file) {
  ...
  // Open Parquet file reader and check for errors
  std::unique_ptr<parquet::arrow::FileReader> arrow_reader;

  arrow::Status status = parquet::arrow::OpenFile(input, pool, &arrow_reader);
  if (!status.ok()) {
    return status;  // Return the error status if file opening fails
  }
